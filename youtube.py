...
def generate_download_link(url):
    try:
        ...
        return None
def generate_mp3_download_link(url):
    try:
        youtube = YouTube(url)
...
def download_mp3_from_links(urls, output_path, is_local):
    try:
        total_videos = len(urls)
        for i, url in enumerate(urls, start=1):
            st.write(f"Downloading video {i} of {total_videos}")
            if is_local and output_path:
                ...
                download_link = generate_mp3_download_link(url)
                if download_link:
                    st.write(f"Download link: {download_link}")
    except Exception as e:
        st.error(f"An error occurred while downloading the video: {str(e)}")
...
def main():
    ...
    elif urls and not mp3:
        download_video_from_links(urls, output_path, is_local)
    elif urls and mp3:
        download_mp3_from_links(urls, output_path, is_local)
